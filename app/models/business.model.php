...
class businessModel {
    ...
    function deleteClothesById($id) {
        $query = $this->db->prepare('DELETE FROM clothes WHERE id_clothes = ?');
        $query->execute([$id]);
    ...
    function upDateClothesById($id){
        $sentencia= $this-> db->prepare("SELECT *FROM clothes WHERE id_clothes =?;");
    ...
    function editEnd2( $des2,$mar2,$tal2,$col2,  $sto2, $pre2,$id) {
        $sentencia=$this->db->prepare('UPDATE clothes SET  description = ?, brand= ?, size = ?, colour=?,stock= ?, price=?  WHERE id_clothes = ?; ');
        $sentencia->execute ([$des2, $mar2, $tal2, $col2,  $sto2, $pre2, $id]);
